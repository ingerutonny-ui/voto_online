...
def reporte():
    ...
    votos_p = db.session.query(Partido.nombre, Partido.ciudad, func.count(Voto.id).label('t')).join(Voto).group_by(Partido.id).all()
    resultados = [{'nombre': r.nombre, 'ciudad': r.ciudad, 'total': r.t, 'porcentaje': round((r.t/total*100), 2) if total > 0 else 0} for r in votos_p]
